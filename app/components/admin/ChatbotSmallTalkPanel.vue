...
<template>
  <section class="flex flex-col gap-5" aria-labelledby="small-talk-panel-title">
    ...
    <div v-if="error" class="rounded-lg border border-[#f1b8b5] bg-[#fff4f3] p-3 text-sm text-[#a32924]" role="alert"><i class="fa-solid fa-triangle-exclamation mr-1.5" aria-hidden="true"></i>{{ error }}<button type="button" class="ml-1.5 font-bold text-[#2c6e33] underline" @click="load(page)">Thử lại</button></div>
    <SkeletonTable v-if="loading" label="Đang tải kho trả lời thường nhật" :rows="6" :cols="7" />
