  ...
  <section class="section bg-white">
    <div class="container">
      <div class="text-center max-w-[600px] mx-auto mb-[50px]">
        ...
        <h2 class="text-[2.2rem] md:text-[1.6rem] font-extrabold text-[#1E251C] mb-3">{{ d.title || 'Mô Hình Tái Hòa Nhập Cộng Đồng' }}</h2>
...
const { data } = await useAsyncData(
  `block-reintegration-${props.block.id}`,
  () => $fetch('/api/public/articles', { params: { type: 'reintegration_model', limit: maxItems.value } }),
  { default: () => ({ articles: [] }) }
)
