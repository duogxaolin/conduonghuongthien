  ...
  <div class="py-10 bg-[#F8FAF7]">
    <div class="max-w-[800px] mx-auto px-4">
      ...
      <article v-else-if="article">
        <span class="text-[0.85rem] font-bold text-[#4A6741] bg-[#F8FAF7] px-3 py-1.5 rounded inline-block mb-4">💡 {{ article.categoryName || 'Mô hình tiêu biểu' }} • Ngày đăng: {{ formattedDate }}</span>
        <h1 class="text-[2.2rem] font-extrabold leading-[1.3] text-[#1E251C] mb-5">{{ article.title }}</h1>

        <div v-if="article.excerpt" class="text-[1.15rem] font-semibold text-[#4A5545] leading-relaxed border-l-4 border-[#7CB342] pl-5 mb-8">
          <p>{{ article.excerpt }}</p>
        </div>

        <div class="article-body">
          <div class="my-8 text-center" v-if="article.thumbnailUrl">
            <img :src="article.thumbnailUrl" :alt="article.title" class="w-full max-h-[450px] object-cover rounded-md shadow-sm"  loading="lazy" decoding="async" />
          </div>
...
useSeoMeta({
  title: computed(() => article.value ? `${article.value.title} | Con Đường Hướng Thiện` : 'Mô hình tái hòa nhập | Con Đường Hướng Thiện'),
  description: computed(() => article.value?.excerpt || 'Mô hình tiêu biểu hỗ trợ người hoàn lương tái hòa nhập cộng đồng.'),
})
</script>
