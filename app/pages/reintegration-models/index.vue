...
const { data: articlesData, pending, error, refresh } = useAsyncData('reintegration-articles', () =>
...
// Most Read
const { data: mostReadData, pending: mostReadPending } = useAsyncData('reintegration-most-read', () =>
  ($fetch as (u: string, o?: Record<string, unknown>) => Promise<{ ok: boolean; articles: Array<{ id: number; title: string; excerpt: string; thumbnailUrl: string; createdAt: string; slug: string; categoryName?: string | null; categorySlug?: string | null }>; pagination: { page: number; limit: number; total: number; totalPages: number } }>)(`/api/public/articles`, { query: { type: 'reintegration', limit: 6, sort: 'views' } }),
  {
    lazy: true,
