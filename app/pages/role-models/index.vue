        ...
        <div class="w-full lg:w-80 shrink-0">
          <form @submit.prevent="applySearch" class="relative flex items-center">
            ...
            <input
              v-model="searchInput"
              type="text"
              :placeholder="t('rm_search_placeholder')"
              :aria-label="t('rm_search_aria')"
            ...
            <button
              ...
              :title="t('rm_clear_search')"
              :aria-label="t('rm_clear_search')"
            >
              ✕
            </button>
            <button
      ...
      <div class="container grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_330px] xl:grid-cols-[minmax(0,1fr)_350px] lg:gap-8 xl:gap-10 lg:items-start">
        ...
        <div class="flex flex-col gap-7 min-w-0">
          ...
          >
            <div class="flex items-center gap-2.5">
              ...
              <span class="text-sm text-[#3E4D3C]">
                {{ t('rm_search_result') }} <strong>&laquo;{{ searchQuery }}&raquo;</strong>
                <span v-if="!pending" class="text-[#7A8A76] ml-1.5">({{ t('rm_articles_count').replace('{n}', String(pagination.total)) }})</span>
              </span>
            </div>
            <button
              type="button"
          ...
          <div v-if="pending" role="status" aria-busy="true" class="flex flex-col gap-8">
            <span class="sr-only">{{ t('rm_list_loading') }}</span>
            <!-- Hero skeleton -->
          ...
          >
            ...
            <h3 class="text-base font-extrabold text-[#992222] m-0 mb-1">{{ t('rm_load_error_title') }}</h3>
            <p class="text-sm text-[#667768] m-0 mb-4">{{ t('rm_load_error_desc') }}</p>
            <button
              type="button"
          ...
          >
            ...
            <h3 class="text-base font-extrabold text-[#233520] m-0 mb-1">{{ t('rm_empty_title') }}</h3>
          ...
          <template v-else>
            ...
            <div v-if="rest.length" class="flex items-center justify-between pt-3 pb-3 border-b-2 border-[#E1EADF] mt-1 mb-5">
              <div class="flex items-center gap-2.5">
                ...
                <span class="text-xs font-extrabold px-2.5 py-0.5 rounded-full bg-[#EBF3E8] text-[#385932]">
                  {{ t('rm_articles_count').replace('{n}', String(pagination.total)) }}
                </span>
              </div>
            ...
            <div v-if="rest.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              >
                <nuxt-link :to="`/role-models/${item.slug}`" class="block relative w-full aspect-video overflow-hidden bg-[#EEF4EC] no-underline shrink-0">
                  <img
                ...
                <div class="p-4 sm:p-5 flex flex-col flex-1">
                  ...
                  <h3 class="text-[0.95rem] sm:text-[0.98rem] font-bold leading-snug text-[#172516] group-hover:text-[#2D5A27] transition-colors duration-200 line-clamp-2 min-h-[2.6rem] mb-2 m-0">
                    <nuxt-link :to="`/role-models/${item.slug}`" class="text-[#172516] hover:text-[#2D5A27] no-underline">
                      {{ item.title }}
                    </nuxt-link>
                  </h3>
                  ...
                  <div class="mt-auto pt-3 border-t border-[#F0F5EE] flex items-center justify-between text-xs text-[#7A8A76]">
                    ...
                    <span class="text-[#8E9F8B] font-medium text-[0.72rem]">{{ t('rm_card_footer_badge') }}</span>
                  </div>
                </div>
              </article>
            </div>
...
useSeoMeta({
  title: () => `${t('rm_title')} | Con Đường Hướng Thiện`,
...
const featured = computed(() => (isLeadPage.value ? roleModels.value[0] ?? null : null))
const rest = computed(() => (isLeadPage.value ? roleModels.value.slice(1) : roleModels.value))

const formatDate = (item: { publishedAt?: string | null; createdAt?: string | null }) =>
  formatDateVN(item.publishedAt || item.createdAt)

// Category Badge: Màu sắc đặc trưng cho tấm gương tiêu biểu
const categoryBadge = (item: { categorySlug?: string | null; categoryName?: string | null } | null | undefined) => {
  if (!item) return { label: t('rm_card_badge'), bg: 'bg-[#2D5A27]', icon: 'fa-solid fa-award' }
  const label = item.categoryName || t('rm_card_badge')
  return { label, bg: 'bg-[#2D5A27]', icon: 'fa-solid fa-award' }
}
