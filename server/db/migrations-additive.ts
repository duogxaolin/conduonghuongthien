/**
 * Di trú bổ sung (additive migrations) — phần "hội tụ lược đồ đang chạy về
 * lược đồ mong muốn", tách khỏi DDL khởi tạo ở `init.ts`.
 *
 * Hai phần này trả lời hai câu hỏi khác nhau và **không** thay thế nhau. `init.ts`
 * dựng bảng cho một CSDL trống (`CREATE TABLE IF NOT EXISTS`). Tệp này lo một
 * CSDL **đã có dữ liệu của cơ quan trên đó**: thêm cột còn thiếu, gắn index,
 * dựng khoá ngoại, và điền giá trị cho hàng cũ trước khi đặt `NOT NULL`. Nhầm
 * chỗ giữa hai phần là một câu `CREATE TABLE` không bao giờ chạy trên máy chủ
 * đang phục vụ, hoặc một câu `ALTER` chạy trên bảng vừa mới tạo.
 *
 * **Thứ tự trong `applyAdditiveMigrations` là một ràng buộc thật**, không phải
 * cách sắp xếp cho dễ đọc: một khoá ngoại phải đợi cột nó trỏ tới, một
 * `NOT NULL` phải đợi lượt điền giá trị, và `releaseChatbotForeignKeysBlocking-
 * ColumnChanges` phải chạy **trước** các lượt đổi cột mà nó mở đường cho. Đó là
 * lý do hàm đó ở đây nguyên khối chứ không bị chia nhỏ thêm theo bảng.
 *
 * `init.ts` re-export toàn bộ tệp này, nên bốn suite test đang import từ
 * `server/db/init` không phải đổi đường import.
 */
...
export async function convergeChatbotSchema(db: Connection, database: string) {
...
export async function hasColumn(db: Connection, database: string, table: string, column: string): Promise<boolean> {
  const [rows] = await db.execute<RowDataPacket[]>(
...
export async function ensureColumn(db: Connection, database: string, table: string, column: string, definition: string) {
  const [rows] = await db.execute<RowDataPacket[]>(
    'SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND COLUMN_NAME = ? LIMIT 1',
    [database, table, column],
  )
...
export async function ensureIndex(db: Connection, database: string, table: string, indexName: string, definition: string) {
  const [rows] = await db.execute<RowDataPacket[]>(
 ...
 */
export async function ensureForeignKeyIfMissing(db: Connection, database: string, table: string, name: string, definition: string) {
  const [rows] = await db.execute<RowDataPacket[]>(
