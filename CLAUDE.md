...
- **Typecheck mới ở chế độ báo cáo.** `npm run typecheck` (→ `nuxt typecheck`) và job CI `Types (report only)` đã có. `typescript`/`vue-tsc` **cố ý chưa** nằm trong `package.json`: CI tự cài lúc chạy, vì thêm devDependency mà không sinh lại `package-lock.json` sẽ làm `npm ci` gãy ở mọi job khác. Job đặt `continue-on-error: true` — lần chạy đầu trên một codebase chưa từng kiểm kiểu chắc chắn có lỗi tồn đọng, và một cổng đỏ ngay từ đầu thì người ta bỏ qua chứ không sửa. Số lỗi in ở summary của job; khi về 0 thì bỏ `continue-on-error`. Muốn cài hẳn: `npm i -D vue-tsc typescript@^5` (lệnh này cập nhật lockfile) rồi xoá bước cài trong workflow. **Ghim TypeScript ^5**: bản 7.x là bản viết lại bằng Go, không còn export `lib/tsc.js` mà vue-tsc cần lúc khởi động.
